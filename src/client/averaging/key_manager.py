...
GroupKey = str
GROUP_PATTERN = re.compile('^(([^.])+)[.]0b[01]*$')
...
class GroupKeyManager:
    ...
    async def get_averagers(self, group_key: GroupKey, only_active: bool) -> List[Tuple[Endpoint, DHTExpiration]]:
        ...
        if result is None or not isinstance(result.value, dict):
            ...
            return []
        averagers = [(key, entry.expiration_time) for key, entry in result.value.items() if
                     key != self.RESERVED_KEY_FOR_NBITS and (not only_active or entry.value is True)]
    ...
    def get_suggested_nbits(cls, search_result: Optional[ValueWithExpiration]) -> Optional[int]:
        if isinstance(search_result,
                      ValueWithExpiration) and cls.RESERVED_KEY_FOR_NBITS in search_result.value and isinstance(
            search_result.value[cls.RESERVED_KEY_FOR_NBITS].value, int):
    ...
    async def notify_stragglers(self):
        ...
        if isinstance(root_data, dict) and root_data.get(self.RESERVED_KEY_FOR_NBITS, (None, -float('inf')))[
            1] > get_dht_time() + self.nbits_grace_period:
            return
