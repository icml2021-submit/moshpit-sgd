...
from src.utils import serialize_torch_tensor, deserialize_torch_tensor, split_for_streaming, combine_from_streaming
...
class AllReduceProtocol:
    def __init__(self, *, group_id: GroupID, tensors: Sequence[torch.Tensor], endpoint: Endpoint,
        ...
        self.accumulator = self.local_tensor_parts[self.endpoint].clone()
        self.accumulated_from: Set[Endpoint] = {self.endpoint}
...
class AllReduceRunner(AllReduceProtocol, averaging_pb2_grpc.DecentralizedAveragingServicer):
    ...
    async def _communicate_with_peer(self, peer_endpoint: Endpoint, local_part: torch.Tensor) -> torch.Tensor:
        ...
        if code != averaging_pb2.AVERAGED_PART:
            raise AllreduceException(
                f"peer {peer_endpoint} returned {averaging_pb2.MessageCode.Name(code)}" f" instead of {averaging_pb2.MessageCode.Name(averaging_pb2.AVERAGED_PART)}," f" allreduce failed")
    ...
    async def run(self) -> Sequence[torch.Tensor]:
        try:
            await asyncio.gather(self, *(self._communicate_with_peer(peer, part) for peer, part in
                                         self.local_tensor_parts.items() if peer != self.endpoint))
            return await self
