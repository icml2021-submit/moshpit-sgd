...
import grpc
import torch
from src.client.averaging.allreduce import AllReduceRunner
...
from src.utils import get_logger, Endpoint, TensorDescriptor, MSGPackSerializer, timed_storage, TimedStorage
...
class Matchmaking(averaging_pb2_grpc.DecentralizedAveragingServicer):
    def __init__(self, endpoint: Endpoint, averaged_tensors: Sequence[torch.Tensor], dht: DHT, *, prefix: str,
                 target_group_size: int, min_group_size: int, initial_group_bits: Optional[str] = None,
        ...
        if request_timeout is None or request_timeout >= averaging_expiration:
            logger.warning(
                "It is recommended to use request_timeout smaller than averaging_expiration. Otherwise," "matchmaking can cause deadlocks in some rare cases. Please see Matchmaking docstring.")
        super().__init__()
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__}(endpoint={self.endpoint}, schema={schema_hash_repr}, {lfg_status}" f" current key = {self.group_key_manager.current_key})"

    async def look_for_group(self, *, data_for_gather: bytes = b'', timeout: Optional[float] = None) -> Optional[
        AllReduceRunner]:
        if self.is_looking_for_group:
            logger.info(
                "Another look_for_group is already in progress. The current run will be scheduled after" " the existing group is either assembled or disbanded.")
    ...
    async def _request_join_potential_leaders(self, timeout: Optional[float]) -> AllReduceRunner:
        async with self.potential_leaders.begin_search(self.group_key_manager, timeout):
            while True:
                try:
                    next_leader = await self.potential_leaders.pop_next_leader()
        ...
        try:
            async with self.lock_request_join_group:
                ...
                call = leader_stub.rpc_join_group(
                    averaging_pb2.JoinRequest(endpoint=self.endpoint, schema_hash=self.schema_hash,
    ...
    async def rpc_join_group(self, request: averaging_pb2.JoinRequest, context: grpc.ServicerContext) -> AsyncIterator[
        ...
        try:
            ...
            if not self.assembled_group.done() and not self.was_accepted_to_group.is_set():
                async with self.lock_request_join_group:
                    if self.assembled_group.done():
                        ...
                        await self.leader_disband_group()
            if self.was_accepted_to_group.is_set() or not self.assembled_group.done() or self.assembled_group.cancelled() or request.endpoint not in self.assembled_group.result():
            ...
            yield averaging_pb2.MessageFromLeader(code=averaging_pb2.BEGIN_ALLREDUCE, group_id=allreduce_group.group_id,
                                                  ...
                                                  gathered=allreduce_group.gathered,
                                                  group_key_seed=allreduce_group.group_key_seed)
        except Exception as e:
            logger.exception(e)
    ...
    def _check_reasons_to_reject(self, request: averaging_pb2.JoinRequest) -> Optional[averaging_pb2.MessageFromLeader]:
        ...
        if request.ListFields() == 3 and not isinstance(request.schema_hash, bytes) or len(
                request.schema_hash) == 0 or not isinstance(request.expiration, DHTExpiration) or not isfinite(
            request.expiration) or not isinstance(request.endpoint, Endpoint) or len(request.endpoint) == 0:
    ...
    async def begin_search(self, key_manager: GroupKeyManager, timeout: Optional[float]):
        async with self.lock_search:
            ...
            try:
                ...
                for field in (
                        self.past_attempts, self.leader_queue, self.running, self.update_finished,
    ...
    async def _update_queue_periodically(self, key_manager: GroupKeyManager):
        try:
            ...
            while get_dht_time() < self.search_end_time:
                ...
                await asyncio.wait({self.running.wait(), self.update_triggered.wait()},
                                   return_when=asyncio.ALL_COMPLETED,
                                   timeout=self.search_end_time - get_dht_time() if isfinite(
                                       self.search_end_time) else None)
...
def compute_schema_hash(tensors: Sequence[torch.Tensor]) -> bytes:
    schema_dicts = [{field_name: str(field_value) for field_name, field_value in
                     asdict(TensorDescriptor.from_tensor(tensor)).items()} for tensor in tensors]
