...
class MSGPackSerializer(SerializerBase):
    ...
    def ext_serializable(cls, type_code: int):
        ...
        def wrap(wrapped_type: type):
            assert callable(getattr(wrapped_type, 'packb', None)) and callable(getattr(wrapped_type, 'unpackb',
                                                                                       None)), f"Every ext_type must have 2 methods: packb(self) -> bytes and classmethod unpackb(cls, bytes)"
