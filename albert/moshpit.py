...
import numpy as np
import src
from src import DHT, get_dht_time, run_in_background, ValueWithExpiration
from src.utils.grpc import CompressionType
...
class CollaborativeTrainer(ExtendableTrainer):
    ...
    def initialize_dht_and_averager(self, collaboration_args: CollaborationArguments):
        ...
        dht = DHT(initial_peers=list(collaboration_args.initial_peers), start=True)
    ...
    def averager_step(self):
        ...
        info = dict(samples_accumulated=self.local_samples_accumulated,
    ...
    def on_train_end(self, *args, **kwargs):
        ...
        self.dht.store(self.training_progess_key, subkey=self.trainer_uuid, value=None,
                       expiration_time=get_dht_time() + self.collaboration_args.metadata_expiration)
    ...
    def check_collaboration_state_periodically(self):
        ...
        while self.is_alive:
            try:
                with self.lock:
                    self.collaboration_state = self.fetch_collaboration_state()
                time.sleep(max(0., self.collaboration_state.next_fetch_time - get_dht_time()))
