...
def main():
    ...
    dht = src.DHT(initial_peers=args.initial_peers, start=True, listen_on=args.dht_listen_on)
    averaged_tensors = [p.detach().requires_grad_(False).cpu().float() for p in model.parameters()]
    averager = src.DecentralizedAverager(
    ...
    src.run_in_background(run_averaging_forever)
